...
namespace Restless.Controls.ResponseActions
{
    public static class ResponseActionRegistry
    {
        ...
        private static readonly List<Tuple<Func<ApiResponseModel, bool>, Type>> actionsWithPredicate = new List<Tuple<Func<ApiResponseModel, bool>, Type>>();
        ...
        static ResponseActionRegistry()
        {
            foreach (var type in typeof(ResponseActionRegistry).Assembly.GetTypes().Where(x => typeof(IResponseAction).IsAssignableFrom(x)))
            {
                ...
                if (predicateMethod != null)
                {
                    ...
                    RegisterAction(type, x => (bool)predicateMethod.Invoke(null, new[] { x }));
                }
            }            
        }

        public static void RegisterAction(Type type, Func<ApiResponseModel, bool> predicate)
        {
            actionsWithPredicate.Add(Tuple.Create(predicate, type));
        }

        public static IEnumerable<IResponseAction> GetActions(ApiResponseModel model)
        {
